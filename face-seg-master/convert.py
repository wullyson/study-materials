...
def change():
    picture="./data/face/Un2.jpg"
    index=[1,2,98,168,327]
    bot=[]
    up=[]
    right=[]
    left=[]
    ...
    v1 = np.array([10, 0])
    v2 = np.array([x[1] * cutface.shape[1]-x[98] * cutface.shape[1],
                   y[1] * cutface.shape[0]-y[98] * cutface.shape[0]])
    dot_product = np.dot(v1, v2)
    norm_v1 = np.linalg.norm(v1)
    norm_v2 = np.linalg.norm(v2)
    cos_theta = dot_product / (norm_v1 * norm_v2)
    ...
    v1 = np.array([-10, 0])
    v2 = np.array([x[1] * cutface.shape[1]-x[327] * cutface.shape[1],
                   y[1] * cutface.shape[0]-y[327] * cutface.shape[0]])
    dot_product = np.dot(v1, v2)
    norm_v1 = np.linalg.norm(v1)
    norm_v2 = np.linalg.norm(v2)
    cos_theta = dot_product / (norm_v1 * norm_v2)
